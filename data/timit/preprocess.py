...
if len(sys.argv) != 3:
    print('Usage python preprocess.py <timit directory> <output path>')
    sys.exit(0)
...
def getpath_insensitive(path):
    ...
    for f in os.listdir(directory):
        ...
        if (os.path.isfile(newpath) or
                os.path.isdir(newpath)) and f.lower() == filename:
            return newpath


train_source_path = getpath_insensitive(os.path.join(paths, 'train'))
test_source_path = getpath_insensitive(os.path.join(paths, 'test'))
target_path = sys.argv[2]

# 61 different phonemes
phonemes = [
    "b", "bcl", "d", "dcl", "g", "gcl", "p", "pcl", "t", "tcl", "k", "kcl",
    "dx", "q", "jh", "ch", "s", "sh", "z", "zh", "f", "th", "v", "dh", "m", "n",
    "ng", "em", "en", "eng", "nx", "l", "r", "w", "y", "hh", "hv", "el", "iy",
    "ih", "eh", "ey", "ae", "aa", "aw", "ay", "ah", "ao", "oy", "ow", "uh",
    "uw", "ux", "er", "ax", "ix", "axr", "ax-h", "pau", "epi", "h#"
]
...
def get_total_duration(file):
    """Get the length of the phoneme file, i.e. the 'time stamp' of the last
    phoneme
    """
...
def create_mfcc(filename):
    ...
    Output consists of 12 MFCC and 1 energy, as well as the first derivative of
    these. [1 energy, 12 MFCC, 1 diff(energy), 12 diff(MFCC)
    """
    sph = SPHFile(filename)
    (rate, sample) = sph.format['sample_rate'], sph.content
...
def preprocess_dataset(source_path):
    ...
    for dirName, subdirList, fileList in os.walk(source_path):
        for fname in fileList:
            ...
            if not fname.lower().endswith(phn_file_postfix.lower()):
                continue

            phn_fname = getpath_insensitive(dirName + '/' + fname)
            wav_fname = getpath_insensitive(dirName + '/' + fname[0:-4] +
                                            RIFF_wav_postfix)
            ...
            for line in fr:
                ...
                phoneme_num = phonemes2index[
                    phoneme] if phoneme in phonemes2index else -1
                end_ind = int(
                    np.round(end_time / total_duration * total_frames))
...
print(
    'Collected {} training instances from {} (should be 4620 in complete TIMIT )'.
    format(train_size, train_source_path))
print(
    'Collected {} testing instances from {} (should be 1680 in complete TIMIT )'.
    format(len(X_test), test_source_path))

print('Spliting {} out of {} ({}%) training data as validation set.'.format(
    int(train_size * val_split), train_size, val_split * 100))
val_idx = [
    int(i)
    for i in random.sample(range(0, train_size), int(train_size * val_split))
]
...
print()
print('Preprocessing completed in {:.3f} secs.'.format(timeit.default_timer() -
                                                       program_start_time))
